...
- `ramsac_angle` is a separate field, shown on the flip side of the story card, not mixed into the glance or the bullets. Write it as a talking point a sales rep could actually use in a client conversation: a question to open with, a line to say verbatim, or a reassurance to give when the client raises the story. Two to three sentences, connected to one of the four angles (managed IT, cybersecurity, Microsoft 365, IT strategy). Test: could an RM read this aloud in a meeting and have it land naturally? If it reads like market commentary rather than something a person would say to a client, rewrite it. This is the only place sales framing belongs.
